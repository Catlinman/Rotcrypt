...
public class Rotator{
	...
	public String rotate(String s, boolean reverse){
		...
		for(int i = 0; i < s.length(); i++){
			...
			if(reverse) currentRot = -currentRot;
			
			String currentLetter = Character.toString(s.charAt(i));
			
			if(debug) System.out.print(currentLetter + " --> ");
			...
			if(currentLetter.matches(".*[abcdefghijklmnopqrstuvwxyz].*")){
				...
				rotatedString = new StringBuilder().append(rotatedString).append(Character.toString(letters.charAt((int) rotatedIndex))).toString();
				
				if(debug) System.out.print(Character.toString(letters.charAt((int) rotatedIndex)) + "\n");
		    }
				...
				rotatedString = new StringBuilder().append(rotatedString).append(Character.toString(upperletters.charAt((int) rotatedIndex))).toString();
				
				if(debug) System.out.print(Character.toString(upperletters.charAt((int) rotatedIndex)) + "\n");
			}
			
			else if(currentLetter.matches(".*[ ].*")){
				rotatedString = new StringBuilder().append(rotatedString).append(" ").toString();
				if(debug) System.out.print("SPACE" + "\n");
			}
			else{
				rotatedString = new StringBuilder().append(rotatedString).append(currentLetter).toString();
				if(debug) System.out.print(currentLetter + "\n");
			}
		}
		if(debug)  System.out.print(rotatedString);
